...
#[deprecated(
    note = "please use `send_from_iterator` or `send_from_iterator_with_transformation` instead"
)]
...
where
    ...
    TDest: Send + Sync + Debug + 'static,
{
    send_from_iterator_with_transformation(src, mapping).await
}

pub async fn send_from_iterator_with_transformation<TSrc, TDest>(
    src: impl Iterator<Item = TSrc> + Send + Sync + 'static,
    mapping: impl Fn(TSrc) -> TDest + Send + Sync + 'static,
) -> Result<
    tonic::Response<
...
pub async fn send_from_iterator<TSrc, TDest: From<TSrc>>(
    src: impl Iterator<Item = TSrc> + Send + Sync + 'static,
) -> Result<
    tonic::Response<
        Pin<
...
where
    TSrc: Send + Sync + Debug + 'static,
    TDest: From<TSrc> + Send + Sync + Debug + 'static,
{
    let channel_size = 100;
